...
import { deviceCookieName } from "@/lib/tokens";
...
export default async function AccessPage({
  ...
  // The link itself is only a secret while it's only ever been opened by
  // one device — otherwise it's just a shareable URL that skips payment.
  // The *first* visit (deviceToken not set yet) claims it via a redirect
  // that sets a cookie; every later visit must present that same cookie.
  ...
  if (submission.deviceToken !== deviceCookie) {
    return (
      <div className="min-h-screen">
        <Nav logoUrl={product.logoUrl} name="Direct2hub" />
        <main className="mx-auto max-w-md px-4 py-16 text-center">
          <div className="card space-y-3 p-6">
            ...
            <p className="text-sm text-brick-700/80">
              This download link is tied to the device that first opened it, to keep it from being shared or
              forwarded. If this is your purchase and you're opening it from a new device, contact support with the
              email or WhatsApp number you used at checkout and we'll help you regain access.
            </p>
            <a href="/" className="inline-block rounded-lg bg-ember-600 px-4 py-2 text-sm font-semibold text-white">
              Back to home
            </a>
